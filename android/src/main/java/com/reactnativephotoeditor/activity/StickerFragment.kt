...
class StickerFragment : BottomSheetDialogFragment() {
  ...
  fun setData(stickerList: ArrayList<String>) {
    println("------------- this.data "+this.data)
    println("------------- stickerList "+stickerList)
  ...
  override fun setupDialog(dialog: Dialog, style: Int) {
    ...
    stickerAdapter.stickerList = this.data.filter {
      sticker -> "green" in sticker
    }
    rvSticker.adapter = stickerAdapter


    //Setup the color picker for text color
    val addTextColorPickerRecyclerView: RecyclerView = contentView.findViewById(R.id.add_sticker_color_picker_recycler_view)
    val layoutManager = LinearLayoutManager(activity, LinearLayoutManager.HORIZONTAL, false)
    addTextColorPickerRecyclerView.layoutManager = layoutManager
    addTextColorPickerRecyclerView.setHasFixedSize(true)
    val colorPickerAdapter = StickerColorPickerAdapter(requireActivity())

    //This listener will change the text color when clicked on any color from picker
    colorPickerAdapter.setOnColorPickerClickListener { colorCode ->
      mColorCode = colorCode
      val hexColor = java.lang.String.format("#%06X", 0xFFFFFF and mColorCode)
      var color: String? = null
      println("----------- mColorCode "+mColorCode+"  "+hexColor)
        when(hexColor){
          "#01A368"-> {
            println("---------------green "+ this.data)
            color="green"
          }
          "#ED0A3F"-> {
            println("---------------red")
          ...
          println("------------ color not available")
          }
        }

      var stickerList: List<String> = this.data.filter {
        sticker -> color.toString() in sticker
      }
      println("---------------stickerList "+ stickerList)
...
//    mColorCode = requireArguments().getInt(TextEditorDialogFragment.EXTRA_COLOR_CODE)
